...
function onnetmessage(data){
		...
		case 1:  // game control messages
			...
			case "attack":
				...
				hexgrid.matrix[parseInt(keywords[2])][parseInt(keywords[3])].piece.setcd(5);
				break;
			case "die":
		...
		if (minimap.checkClick(event)) {
			...
			if (coord) {
				...
				if (last_click_coord && isReachable) { // move a unit to a reachable coord
					socket.send('1 move ' + last_click_coord.X +' ' + last_click_coord.Y + ' ' + coord.X + ' ' + coord.Y);
					last_click_coord = null;
					hexgrid.clearReachable();
				}
			    else if (last_click_coord && isAttackable){
					socket.send('1 attack ' + last_click_coord.X +' ' + last_click_coord.Y + ' ' + coord.X + ' ' + coord.Y);
					last_click_coord = null;
					hexgrid.clearReachable();
