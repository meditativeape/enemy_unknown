...
*/
    ...
    var game_player = function( game_instance, player_instance ) {
        ...
        if(player_instance) {
            this.pos = { x:20, y:20 };
			this.isGhost = true;  // host is the ghost by default
        } else {
    ...
    game_player.prototype.draw = function(){
        ...
        game.ctx.fillStyle = this.color; 
            ...
            //Draw a rectangle/circle for us
		if (this.isGhost) {
			game.ctx.beginPath();
			game.ctx.arc(this.pos.x, this.pos.y, this.size.hx, 0, 2*Math.PI, true);
			game.ctx.fill();
		} else {
...
game_core.prototype.server_update_physics = function() {
    ...
    this.check_collision( this.players.other );
		...
		// check if the ghost has caught the sheep
	this.server_check_catch();
...
game_core.prototype.server_update = function(){
    ...
    this.laststate = {
		...
		hig : this.players.self.isGhost,            //'host is ghost'
		cig : this.players.other.isGhost,           //'client is ghost'
		hii : this.players.self.impregnable,        //'host is impregnable'
		cii : this.players.other.impregnable,       //'client is impregnable'
...
game_core.prototype.server_check_catch = function() {

	if (this.players.self.isGhost) {
		ghost = this.players.self;
		sheep = this.players.other;
	} else {
		ghost = this.players.other;
		sheep = this.players.self;
	}
	
	var current_time = new Date().getTime();
	if (sheep.impregnable && (current_time - sheep.impregnableStartAt) > this.impregnableDuration) {
		//console.log("change!");
		sheep.impregnable = false;
	}
	
	if (!sheep.impregnable && Math.pow(ghost.pos.x - sheep.pos.x, 2) + Math.pow(ghost.pos.y - sheep.pos.y, 2) 
			<= Math.pow(ghost.size.x * 2, 2)) {
		ghost.isGhost = false;
		ghost.impregnable = true;
		ghost.impregnableStartAt = current_time;
		sheep.isGhost = true;
		sheep.freeze = true; // Do we need to freeze the new ghost for a while?
		sheep.freezeStartAt = ghost.impregnableStartAt;
	}
...
game_core.prototype.client_onserverupdate_recieved = function(data){
		...
		// update local players' states
		if (data.hig != undefined) {
			player_host.isGhost = data.hig;
		}
		if (data.cig != undefined) {
			player_client.isGhost = data.cig;
		}
		if (data.hii != undefined) {
			player_host.impregnable = data.hii;
		}
		if (data.cig != undefined) {
			player_client.impregnable = data.cig;
		}
